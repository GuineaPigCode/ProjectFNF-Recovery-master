...
class OptionsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		CoolUtil.coolStartTransition(FlxG.camera);
...
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = ['GRAPHICS', 'GAMEPLAY', 'MODIFIERS'];
	static var noCheckbox:Array<String> = ['Framerate', 'Note Delay', 'Damage from Dad Notes', 'No Health Gain'];
				...
				if (holdTime > 0.5 || controls.UI_LEFT_P || controls.UI_RIGHT_P)
					switch (options[curSelected])
					{
						...
						case 'Damage from Dad Notes':
							...
							if (ClientPrefs.damageFromDadNotes < 5)
								ClientPrefs.damageFromDadNotes = 5;
						...
						case 'No Health Gain':
							...
							if (ClientPrefs.noHealthGain < 0)
								ClientPrefs.noHealthGain = 0;
							else if (ClientPrefs.noHealthGain > 100)
								ClientPrefs.noHealthGain = 100;
					}
				reloadValues();
